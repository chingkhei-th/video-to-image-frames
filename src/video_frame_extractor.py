...
def extract_frames(video_path, frames_folder, fps):
    ...
    print(
        f"Extracted {frame_count // step_size} frames from the video to {output_folder}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "frames_folder",
        help="Path to the frames folder where the new subfolder will be created.",
    )
    ...
    extract_frames(
        data_folder / Path(args.video_path),
        data_folder / Path(args.frames_folder),
        args.fps,
    )
